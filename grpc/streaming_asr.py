...
def preprocessor_normalization(cfg):
	# make config overwritable
...
class StreamingASR:
	...
	def __init__(self, model, model_cfg,
		...
		'''
		self.vocab = list(model_cfg.decoder.vocabulary)
		self.vocab.append('_')
		self.model = model
		self.sr = model_cfg.preprocessor.sample_rate
		self.frame_len = frame_len
		self.n_frame_len = int(frame_len * self.sr)
		self.frame_overlap = frame_overlap
		self.n_frame_overlap = int(frame_overlap * self.sr)
		timestep_duration = model_cfg.preprocessor.window_stride
		for block in model_cfg.encoder.jasper:
		...
		self.offset = offset
		self.reset()
		self.data_layer = AudioDataLayer(sample_rate=model_cfg.preprocessor.sample_rate)
	...
	def _decode(self, frame, offset=0):
		...
		# buffer shifted by n_frame_len to add frame at the end (semi queue)
	...
	def reset(self):
		'''
		Reset frame_history and decoder state
		'''
